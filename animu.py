...
async def danbooru(event):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get("http://danbooru.donmai.us/posts.json", params=params) as response:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            for index, post in enumerate(data[:num_pics]):
                image_url = next(
                    (post.get(url) for url in ["file_url", "large_file_url", "source"] if url in post),
                    None
                )
                if not image_url:
                    continue
