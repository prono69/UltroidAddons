    ...
    try:
        ...
        if response.status_code == 200:
            if query in ["loli", "lolp"]:
                ...
                image_response = requests.get(direct_image_url, stream=True, timeout=120)
                
                if image_response.status_code == 200:
                    # Save the content to a file
                    with open(file_name, "wb") as file:
                        shutil.copyfileobj(image_response.raw, file)
                    return os.path.abspath(file_name)  # Return the full path of the saved file
                else:
            ...
            return f"Failed to retrieve the image URL. Status code: {response.status_code}"
