...
from . import HNDLR, ultroid_cmd

session = aiohttp.ClientSession()

NSFW = ["lick", "kiss", "butts", "andro", "gyno", "lesbian", "straight"]
replacement_map = {"andro": "andromorph", "gyno": "gynomorph"}
...
async def furry(event):
    ...
    if match not in NSFW:
        return await eod(
            event, f"**Please choose a correct category**\nType: `{HNDLR}help furry`"
        )
    ...
    url = (
        f"{base}/yiff/{match}"
        if match not in ("kiss", "butts", "lick")
        else f"{base}/{match}"
    )
    try:
        async with session.get(url) as resp:
            if resp.status == 200:
                ...
                return await eod(
                    event, f"**Failed to fetch image: HTTP Status {resp.status}**", 5
                )
    ...
    sources_text = "\n".join(
        [
            f'<a href="{html.escape(source)}">{html.escape(source)}</a>'
            for source in final_sources
        ]
    )
    caption = (
        f"<b>✘ Artist:</b> <i>{artist_}</i>\n\n<b>✘ Sources:</b>\n<i>{sources_text}</i>"
    )
    await event.delete()
    await event.client.send_file(
        event.chat_id, file=pic, caption=caption, parse_mode="html", reply_to=reply_to
    )
