...
async def analyze_image_cmd(e):
    ...
    prompt = (
        e.pattern_match.group(3) or "Provide a description of the person in the image."
    )
    ...
    try:
        ...
        if len(response_str) > 4096:
            ...
            await e.client.send_file(
                e.chat_id,
                file_name,
                caption="Response was too long, so here it is in a file.",
            )
            remove(file_name)
        else:
