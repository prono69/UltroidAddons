...
async def revert_identity(event):
    ...
    bio = (
        udB.get_key(bio_key)
        if udB.get_key(bio_key) is not None
        else "No original bio saved."
    )
    first_name = (
        udB.get_key(fname_key) if udB.get_key(fname_key) is not None else OWNER_NAME
    )
    ...
    await event.client(
        UpdateProfileRequest(first_name=first_name, last_name=last_name, about=bio)
    )
        ...
        try:
            ...
            if isinstance(mention_entity, MessageEntityMentionName):
                replied_user = await event.client(
                    GetFullUserRequest(mention_entity.user_id)
                )
                return replied_user, None
