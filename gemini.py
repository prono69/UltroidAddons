...
# Available models
available_models = ["models/gemini-1.5-flash", "models/gemini-1.5-flash-latest", "models/gemini-1.0", "models/gemini-2.0-flash", "models/gemini-2.0-flash-latest"]
...
async def gemini_handler(event):
    ...
    if query == "-ls":
        ...
        await processing_message.edit(f"📜 **Available Models:**\n`{models_list}`\n\n**Current Model:** `{current_model_name}`")
        return
    ...
    if query.startswith("-m"):
        ...
        if new_model_name in available_models:
            ...
            model = genai.GenerativeModel(current_model_name)
            await processing_message.edit(f"✅ **Model switched to:** {current_model_name}")
        else:
            await processing_message.edit("⚠️ **Invalid model name! Use `.gemini -ls` to view available models.**")
        return
    ...
    if replied_msg:
        ...
        # If replying to a file
            ...
            try:
                ...
                gemini_file = genai.upload_file(path=file_path, display_name=replied_msg.file.name)
    ...
    try:
        ...
        async with event.client.action(event.chat_id, "typing"):
            ...
            conversation_history[timestamp] = f"Q: {query or '(File Uploaded)'} | A: {response.text}"

            await processing_message.edit(f"🤖 **Gemini's Reply:**\n__{response.text}__")
